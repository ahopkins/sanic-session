...
async def test_should_return_data_from_memcache(
    ...
    assert (
        memcache_connection.get.call_count == 1
...
@pytest.mark.asyncio
async def test_should_delete_session_from_memcache(
    mocker, mock_memcache, mock_dict
):
    request = mock_dict()
    response = mock_dict()
...
@pytest.mark.asyncio
async def test_should_expire_memcache_cookies_if_modified(
    mock_dict, mock_memcache
):
    request = mock_dict()
    response = text("foo")
    request.cookies = COOKIES
...
@pytest.mark.asyncio
async def test_should_save_in_memcache_for_time_specified(
    mock_dict, mock_memcache
):
    request = mock_dict()
    request.cookies = COOKIES
    ...
    memcache_connection.set.assert_called_with(
        "session:{}".format(SID).encode(),
        ujson.dumps(request.ctx.session).encode(),
        exptime=2592000,
    )
